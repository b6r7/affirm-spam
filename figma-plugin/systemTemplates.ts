import type { Locale, TemplateInfo } from '../messageTypes';

/**
 * Legacy system template definitions (schema reference only).
 * Plugin ships with no seeded templates; DEFAULT_APP_STATE uses empty catalog.
 */
export const systemTemplates: TemplateInfo[] = [
  {
    id: 'payment-reminder',
    name: {
...
function emptyResolvedCompliance(): Record<Locale, number[]> {
  const o: Record<string, number[]> = {};
  LOCALES.forEach((l) => (o[l] = []));
  return o as Record<Locale, number[]>;
}

/** Empty catalog so plugin opens "factory new"; users create or import templates. */
const EMPTY_TEMPLATE_CATALOG: TemplateInfo[] = [];

export const DEFAULT_APP_STATE: import('../messageTypes').AppState = {
  locale: 'en-US',
  tone: 'Neutral',
  layoutMode: 'affirm',

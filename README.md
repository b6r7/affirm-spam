...
This repo is a React + Vite UI prototype generated via Figma Make. It currently runs as a web app, but the UI architecture is already close to a plugin shape (canvas preview on the left, control panel on the right). The next step is to wrap the UI into Figma’s plugin runtime and connect actions (generate, export, save template, etc.) to real plugin logic.
...
**Factory-new:** The Figma plugin ships with no seeded templates; users create or import their own. On first run you’ll see an empty state with “Add new” and “Import translations” to get started.
...
**Build order:** UI is built first (`figma-plugin/ui.js`, `figma-plugin/ui.css`), then the main thread build inlines that UI into `code.js` as `__html__`, so Figma’s `showUI(__html__)` has no relative URLs.

**Plugin directory:** Figma loads the plugin from the folder that contains `manifest.json`. Use the **`figma-plugin/`** folder: that’s where `manifest.json`, `code.js`, `ui.html`, `ui.js`, and `ui.css` live after a build.
...
4. The plugin appears under **Plugins** → **Development** → **Affirm SPAM**. Run it to open the sidebar.
...
2. **Re-import in Figma**
   - **Plugins** → **Development** → **Import plugin from manifest…** and select **`figma-plugin/manifest.json`** (the plugin directory is **figma-plugin/**).
3. **Check what’s running**
   - You should see “UI BOOT MARKER” in the plugin window and, in the plugin console, “UI LOADED” and “Plugin UI mounted”. The `<pre id="bootlog">` in the inlined HTML surfaces `window.onerror` and `unhandledrejection` if something fails.
...
- `imports/`
  - Generated SVG/icon imports (example: `🧬AffirmLogoMobile.tsx`)

Reference:
- `ATTRIBUTIONS.md` documents generated assets and attributions.

## Core files you’ll touch most
- `src/App.tsx`
...
- `src/components/CommsForgePanel.tsx`
  ...
  - Enters “create new template” mode via `TemplateCreationPanel`
...
1. Plugin “main” (runs in Figma, has access to Figma API)
2. Plugin “UI” (an iframe running HTML/JS, no direct Figma API access)

### Recommended conversion approach
Keep your current React UI as the plugin UI, and create a small “main” controller that:
...
- `figma-plugin/manifest.json` — Figma plugin manifest; `main` points to `code.js`, `ui` to `ui.html`.
- `src/plugin/code.ts` — Main thread entry; built with esbuild to `figma-plugin/code.js`.
- `src/plugin/ui.tsx` — React UI entry (reuses `AppView`); built with Vite to `figma-plugin/ui.js` + `ui.css`.
- `figma-plugin/ui.html` — Loads `ui.css` and `ui.js`. Shared types/storage/translations live in `figma-plugin/`.
...
For templates, you’ll likely want:
...
- For a plugin-ready build, prefer:
  ...
  - Use Figma’s default UI font stack for the plugin UI.
...
- Template creation is mostly “demo mode” today. Real version should:
...
- When moving to plugin mode:
  ...
  - Add a typed bridge layer early so you don’t end up with stringly-typed message chaos
  - Keep UI logic deterministic (no random IDs without a wrapper function) so it’s testable
